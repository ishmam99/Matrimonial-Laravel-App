            ...
            @foreach($sliders as $slider)
              <tr>
               <td>{{ $loop->iteration + $sliders->firstItem() - 1 }}</td>
                <td><img src="{{setImage($slider->image)}}" width="50px" alt=""></td>
                <td>{{ $slider->name }}</td>
                <td>{{ $slider->status ? 'Unpublished' : 'Published' }}</td>
                <td nowrap="nowrap">
                  <a href="{{ route('slider.edit', $slider->id) }}" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
                  ...
                  <form method="post" action="{{ route('slider.destroy', $slider->id) }}" class="d-inline-block">
                    @csrf
                    @method('DELETE')
