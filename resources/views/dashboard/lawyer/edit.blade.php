   ...
   {{-- @include('dashboard.user.aside') --}}
    ...
    <div class="flex-row-fluid ml-lg-8">
      <div class="card card-custom card-stretch">
        ...
        <form class="form p-5" action="{{ route('lawyer.update',$lawyer->id) }}" id="profile_update_form" method="post" enctype="multipart/form-data">
          @csrf
          @method('PUT')
           <div class="form-group row">
              ...
              <div class="col-lg-9 col-xl-6">
                <div class="image-input image-input-outline image-input-circle" id="kt_image_4">
                  <div class="image-input-wrapper" style="width: 120px; background-position: center; background-image: url({{ setImage($lawyer->profile_photo, 'user') }})"></div>
            ...
            <div class="form-group row">
             ...
             <div class="col-8">
              <input type="text" name="name" class="form-control" value="{{$lawyer->name}}">
            </div>
            </div>
             <div class="form-group row">
              <div class="col-4">
             ...
             <div class="col-8">
              <input type="email" name="email" class="form-control" value="{{$lawyer->email}}">
            </div>
            </div>
             <div class="form-group row">
              <div class="col-4">
             ...
             <div class="col-8">
              <input type="text" name="phone" class="form-control" value="{{$lawyer->phone}}">
            </div>
            </div>
            <div class="form-group row">
              <div class="col-4">
             ...
             <div class="col-8">
              <input type="text" name="fee" class="form-control" value="{{$lawyer->fee}}">
            </div>
            </div>
        </form>
      </div>
      <div class="col-4">
        ...
        <form action="{{route('lawyer.addBadge',$lawyer->id)}}" method="POST">
        @csrf
        <div class="card ">
          <label for="badge_id">Badge</label>
             @if ($lawyer->badge==null)
              <p class="text-danger">No Active Badge</p>
          @endif  
          <img src="{{setImage($lawyer->badge?->badge->badge_image)}}" height="50px" width="50px" alt="">
          <select name="badge_id" id="badge_id" class="form-control">
            
            @foreach (App\Models\Badge::all() as $badge)
                <option value="{{ $badge->id}}" {{ $badge->id==$lawyer->badge?->badge->id ? 'selected' : null }}>{{ $badge->name }}</option>
            @endforeach
          </select>
