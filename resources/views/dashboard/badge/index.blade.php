  ...
  <div class="card card-custom">
    ...
    <div class="card-header flex-wrap border-0 pt-6 pb-0">
      <div class="card-title">
        <h3 class="card-label">Badge List</h3>
      </div>
        ...
        <table class="table table-separate table-head-custom table-checkable" id="kt_datatable">
          ...
          <tbody>
            @foreach($badges as $badge)
              <tr>
                <td>{{ $loop->iteration + $badges->firstItem() - 1 }}</td>
                <td>{{$badge->name}}</td>
                <td><img src="{{setImage($badge->badge_image)}}" alt="" height="50px" width="50px"></td>
                <td>@if ($badge->status==true)
                    <span class="btn btn-success">Active</span>
                    @else
                        <span class="btn btn-danger">Inactive</span>
                @endif</td>
                <td>{{$badge->badgeUser->count()}}</td>
                <td nowrap="nowrap">
                  <a href="{{route('badge.edit',$badge->id)}}" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
