  ...
  <div class="card card-custom">
    ...
    <div class="card-header flex-wrap border-0 pt-6 pb-0">
      <div class="card-title">
        <h3 class="card-label">Course Review List</h3>
      </div>
            ...
            @foreach($reviews as $review)
              <tr>
               <td>{{ $loop->iteration + $reviews->firstItem() - 1 }}</td>
                ...
                <td>{{ Carbon\Carbon::parse($review->created_at)->isoFormat('MMM Do YY') }}</td>
                <td nowrap="nowrap">
                  <a href="{{ route('courseReview.show', $review->id) }}" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-2">
                    <span class="svg-icon svg-icon-md svg-icon-primary">
                      <img src="https://img.icons8.com/ios/20/000000/view-file.png"/>
                    </span>
                  </a>
                  <a href="{{ route('courseReview.edit', $review->id) }}" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
                    <span class="svg-icon svg-icon-md svg-icon-primary">
                  ...
                  <form method="post" action="{{ route('courseReview.destroy', $review->id) }}" class="d-inline-block">
                    @csrf
                    @method('DELETE')
