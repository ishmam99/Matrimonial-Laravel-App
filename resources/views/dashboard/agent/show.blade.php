      ...
      <div class="card card-custom card-stretch">
        <div class="card-header py-3">
          ...
          <div class="card-toolbar">
              <a href="{{route('agent.contract',$agent->id)}}"><button type="submit" class="btn btn-primary mr-2" >Case List</button></a> 
               <a href="{{route('agent.edit',$agent->id)}}"><button type="submit" class="btn btn-success mr-2" >Edit Agent</button></a> 
        ...
        <div class="card-body">
            ...
            <div class="col-6 ">
                ...
                <div class="row ">
                  ...
                  <div class="col-xl-6 col-lg-6 col-form-label text-left">
                    <h5>{{$agent->contract?$agent->contract->where('status',App\Models\UserAgent::RUNNING)->count() : '0'}}</h5>
                  </div>
                  <label class="col-xl-4 col-lg-4 col-form-label" for="name"> Hold Cases : </label>
                  <div class="col-xl-6 col-lg-6 col-form-label text-left">
                    <h5>{{$agent->contract?$agent->contract->where('status',App\Models\UserAgent::HOLD)->count(): '0'}}</h5>
                  </div>
                  <label class="col-xl-4 col-lg-4 col-form-label" for="name"> Pending Cases : </label>
                  <div class="col-xl-6 col-lg-6 col-form-label text-left">
                    <h5>{{$agent->contract?$agent->contract->where('status',App\Models\UserAgent::PENDING)->count():'0'}}</h5>
                  </div>
                  <label class="col-xl-4 col-lg-4 col-form-label" for="name"> Dismissed Cases : </label>
                  <div class="col-xl-6 col-lg-6 col-form-label text-left">
                    <h5>{{$agent->contract?$agent->contract->where('status',App\Models\UserAgent::DISMISSED)->count(): '0'}}</h5>
                  </div>
                  <label class="col-xl-4 col-lg-4 col-form-label" for="name"> Closed Cases : </label>
                  <div class="col-xl-6 col-lg-6 col-form-label text-left">
                    <h5>{{$agent->contract?$agent->contract->where('status',App\Models\UserAgent::CLOSED)->count():'0'}}</h5>
                  </div></div>
            </div></div>  @php
                        $total= $agent->user->reviews->count();
                        $totals = $agent->user->reviews->sum('rating')/5;
                        if($agent->reviews){
            ...
            <div class="col-6">
                <div class="bg-white rounded shadow-sm p-4 mb-4 clearfix graph-star-rating">
                    ...
                    <div class="graph-star-rating-header">
                        <div class="star-rating">
                            ...
                            <a href="#"><i class="icofont-ui-rating active"></i></a>
                            <a href="#"><i class="icofont-ui-rating active"></i></a>
                            <a href="#"><i class="icofont-ui-rating"></i></a> <b class="text-black ml-2">Total {{$agent->user->reviews->count()}}</b>
                        </div>
              ...
              <div class="bg-white rounded shadow-sm p-4 mb-4 restaurant-detailed-ratings-and-reviews col-12">
                    ...
                    @foreach ($agent->user->reviews->load('profile') as $review)
