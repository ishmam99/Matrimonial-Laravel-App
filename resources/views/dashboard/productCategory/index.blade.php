    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label">Product Category List</h3>
            </div>
            <div class="card-toolbar">
                @can('web_content.create')
                    <a href="{{ route('productCategory.create') }}" class="btn btn-primary font-weight-bolder">
                    ...
                    <tbody>
                        @foreach ($productCategories as $productCategory)
                            <tr>
                                ...
                                <td>
                                  <img style="width: 50px;" src="{{ setImage($productCategory->image) }}" alt="">
                                </td>
                                {{-- <td>
                                    {{ $productCategory->message }}
                                </td> --}}
                                <td nowrap="nowrap">
                                    @can('web_content.edit')
                                        <a href="{{ route('productCategory.edit', $productCategory->id) }}" class="btn btn-icon btn-light btn-hover-primary btn-sm mr-2">
                                    ...
                                    @endcan
                                    @can('web_content.delete')
                                        <form method="post" action="{{ route('productCategory.destroy', $productCategory->id) }}" class="d-inline-block">
                                            @csrf
                                            @method('DELETE')
