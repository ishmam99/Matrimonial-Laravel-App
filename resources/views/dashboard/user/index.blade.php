          ...
          <tbody>
            @foreach($users as $user)
              <tr>
                ...
                <td>{{ $user->email }}</td>
                <td nowrap="nowrap">
                  @if ($user->profile || $user->lawyer||$user->kazi )
                      
                 
                  <a href="{{route('user.edit',$user->id)}}" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
                  ...
                  <form method="post" action="{{route('user.destroy',$user->id)}}" class="d-inline-block">
                    ...
                    {{-- @php
                        $profile=route('user.show',$user->id);
                        $lawyer=route('lawyer.show',$user->lawyer->id);
                         $kazi=route('kazi.show',$user->kazi->id);
                        // $agent=route('agent.show',$user->agent->id);
                    @endphp --}}
                  </form>
                  <a href="{{$user->profile ? route('user.show',$user->id) : ($user->lawyer ?route('lawyer.show',$user->lawyer->id) : ($user->kazi ? route('kazi.show',$user->kazi->id) : route('agent.show',$user->agent->id)))}}" class="btn btn-icon btn-light btn-hover-success btn-sm mx-3">
